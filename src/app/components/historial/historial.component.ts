...
export class HistorialComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //list tipos de equipos
    this.dashboardService.getEquipmentTypeEquivalenceList().subscribe({
      next: data => {
        this.resultadoPeticion = data;
        if (this.resultadoPeticion.code == 200) {
          ...
          for (let tipo of this.tiposDeEquipos) {
            this.optionsTipos.push(tipo);
          }
  ...
  getTipo(tipo: TipoDeEquipo) {
    if (tipo != null) {
      //list marcas de equipos
      this.tipoSeleccionado = tipo;
      this.isShowingSpinner = true;
      this.dashboardService.getBrandsByEquipmentTypeList(tipo.id).subscribe({
        next: data => {
          this.resultadoPeticion = data;
          if (this.resultadoPeticion.code == 200) {
            this.marcasDeEquipos = this.resultadoPeticion.data;
            this.optionsMarcas.length = 0;

            for (let marca of this.marcasDeEquipos) {
              this.optionsMarcas.push(marca);
            }
          }
          this.isShowingSpinner = false;
        },
  ...
  getMarca(marca: Marca) {
    if (marca != null && this.tipoSeleccionado != null) {
      ...
      this.isShowingSpinner = true;
      this.dashboardService.getModelByTypeEquivalenceBrand(marca.id, this.tipoSeleccionado.id).subscribe({
        next: data => {
          this.resultadoPeticion = data;
          if (this.resultadoPeticion.code == 200) {
            
            this.modeloDeEquipos = this.resultadoPeticion.data;
            this.optionsModelos.length = 0;

            for (let modelo of this.modeloDeEquipos) {
              this.optionsModelos.push(modelo);
            }
          }
          this.isShowingSpinner = false;
        },
